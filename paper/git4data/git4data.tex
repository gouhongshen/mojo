...
with 64 cores AMD64 CPU, 256~GB memory, and local SSD disks.  The MatrixOne
database system is installed on a Kubernetes cluster.
...
the primary key (the PK test cases), and one without a primary key (NoPK test cases).  

\subsection{Experiment 1: Clone}\label{sec:expClone}
Users can create a new copy of the data by cloning the lineitem table or by
using the following SQL statement:
\begin{verbatim}
  INSERT INTO T AS SELECT * FROM lineitem;
\end{verbatim}
Table \ref{tab:clone} shows the cost of clone vs insert, both in terms of time and storage space.
\begin{table}[h]
  \centering
  ...
  \begin{tabular}{lcc}
    ...
    \midrule
    Clone, PK  & 0.20 & 314.153~KB \\
    Clone, noPK & 0.17 & 294.375~KB \\
    Insert, PK  & 114.63 & 34~GB \\
    Insert, noPK & 119.31 & 34~GB \\
    \bottomrule
  \end{tabular}
\end{table}

Clone operations only copy the metadata for existing objects.  Each metadata file is 157~bytes, so the PK test (2{,}049 objects) reads 314.153~KB while the NoPK test (1{,}920 objects) reads 294.375~KB, matching the space reported in Table \ref{tab:clone}.
In contrast, insert-based copies materialize every row of the TPC-H 100GB lineitem table, so each insert row in Table \ref{tab:clone} occupies roughly 34~GB, the full table size.

\subsection{Experiment 2: Snapshot Diff/Merge}
We generate four change sets of different sizes to table T created in 
Experiment \ref{sec:expClone}. 
\begin{itemize}
\item \textbf{C1}: update 1{,}000 rows (1K).
\item \textbf{C2}: update 10{,}000 rows (10K).
\item \textbf{C3}: update 100{,}000 rows (100K).
\item \textbf{C4}: update 1{,}000{,}000 rows (1M).
\end{itemize}
After applying these changes, we compute the diff between T and the original 
lineitem table, and then perform merge back from T to lineitem using \texttt{ACCEPT} mode.
We compare our built-in snapshot diff/merge operations with an implementation
using SQL.  The diff SQL query is in Listing \ref{lst:diff} and the merge
SQL query is in Listing \ref{lst:merge-sql}.
...
a primary key actually cheated a bit by assuming external knowledge that 
\texttt{(l\_orderkey, l\_linenumber)} is in fact the primary key.  
...
\begin{table}[h]
  \centering
  \caption{BuiltIn vs.\ SQL Time(s)}
  \label{tab:exp2}
  \resizebox{\linewidth}{!}{
    \begin{tabular}{lcccccccc}
      \toprule
      & \multicolumn{4}{c}{BuiltIn} & \multicolumn{4}{c}{SQL} \\
      \cmidrule(lr){2-5} \cmidrule(lr){6-9}
      \textbf{Operation} & \textbf{C1} & \textbf{C2} & \textbf{C3} & \textbf{C4} & \textbf{C1} & \textbf{C2} & \textbf{C3} & \textbf{C4} \\
      \midrule
      Diff, PK  & 0.19 & 0.38 & 1.73 & 3.27 & 316.16 & 418.19 & 428.78 & 431.50 \\
      Diff, NoPK  & 0.85 & 22.50 & 9.04 & 60.19 & 378.19 & 396.61 & 394.31 & 371.74 \\
      Merge, PK & 0.35 & 0.97 & 7.95 & 16.13 & 321.52 & 412.89 & 442.68 & 471.16 \\
      Merge, NoPK & 0.88 & 22.70 & 12.09 & 68.75 & 393.95 & 405.35 & 401.59 & 403.18 \\
      \bottomrule
    \end{tabular}
  }
\end{table}
Built-in operations are much faster than SQL implementations in all cases.
We observe that the performance of the built-in operations does not depend on 
the size of the table, instead, it depends on the size of the changes.
For a change size of 1 million rows, our built-in merge takes 
only seconds, which is good enough for a team of data engineers to perform
PR (pull request) review, CI, and merging PRs. 

\subsection{Experiment 3: Collaborative Workload Without Conflicts}\label{sec:expcollab}
This experiment simulates a collaborative workload when a team of data engineers are 
working on the same dataset.  Four data engineers fork the lineitem table to their 
copy of tables, T1, T2, T3, and T4 respectively.  Each engineer updates 
their own table and in this experiment, they cooperate with each other
so that updates of one engineer do not overlap with those of another.
After all the updates are applied, each engineer merges their changes 
back to the original table.  

Figure \ref{fig:collab-timeline} shows 
...
\end{figure}
Table \ref{tab:collab-avg} shows the average time (s) of four workers.  To fork the tables,
we compare the built-in clone operation with SQL insert statements as in 
section \ref{sec:expClone}.  We want to make a note that using insert will incur four 
times of storage space overhead.

\begin{table}[h]
  \centering
  \caption{Collaborative Snapshot Diff/Merge.  Average Time (s) Of Four Workers}
  \label{tab:collab-avg}
  \resizebox{\linewidth}{!}{
    \begin{tabular}{lcccccccc}
      \toprule
      & \multicolumn{4}{c}{BuiltIn} & \multicolumn{4}{c}{SQL} \\
      ...
      \textbf{Operation} & \textbf{C1} & \textbf{C2} & \textbf{C3} & \textbf{C4} & \textbf{C1} & \textbf{C2} & \textbf{C3} & \textbf{C4} \\
      \midrule
      Fork, PK  & 0.14 & 0.16 & 0.14 & 0.15 & 120.56 & 122.42 & 122.14 & 121.92 \\
      Diff, PK  & 0.08 & 0.28 & 1.36 & 3.04 & 438.41 & 460.33 & 483.35 & 507.52 \\
      Merge, PK  & 0.26 & 1.29 & 7.98 & 22.81 & 421.87 & 442.96 & 465.11 & 488.36 \\
      Fork, NoPK  & 0.13 & 0.15 & 0.15 & 0.15 & 119.41 & 123.35 & 119.94 & 118.72 \\
      Diff, NoPK  & 0.65 & 28.34 & 8.41 & 58.72 & 433.28 & 448.88 & 443.68 & 444.54 \\
      Merge, NoPK  & 0.56 & 28.29 & 11.37 & 66.59 & 421.87 & 437.06 & 431.99 & 432.84 \\
      \bottomrule
    \end{tabular}
  }
\end{table}
...
\begin{figure}[t]
  \centering
  \input{fig_collab_merge.tex}
  \caption{Merge time across change sets for collaborative workload without conflicts.}
  \label{fig:collab-merge-plot}
\end{figure}
...
but with conflicts.  We let updates of two engineers have 10\% 
overlap on the primary key.
Table \ref{tab:collab-conflict-avg} shows the average time (s) of four workers.  
\begin{table}
  \centering
  \caption{Collaborative Snapshot Diff/Merge.  Average Time (s) Of Four Workers}
  \label{tab:collab-conflict-avg}
  \resizebox{\linewidth}{!}{
    \begin{tabular}{lcccccccc}
      \toprule
      & \multicolumn{4}{c}{BuiltIn} & \multicolumn{4}{c}{SQL} \\
      ...
      \textbf{Operation} & \textbf{C1} & \textbf{C2} & \textbf{C3} & \textbf{C4} & \textbf{C1} & \textbf{C2} & \textbf{C3} & \textbf{C4} \\
      \midrule
      Fork, PK  & 0.14 & 0.16 & 0.14 & 0.16 & 125.59 & 124.54 & 120.45 & 118.48 \\
      Diff, PK  & 0.08 & 0.28 & 1.52 & 3.12 & 456.37 & 479.19 & 503.15 & 528.30 \\
      Merge, PK  & 0.15 & 0.64 & 4.06 & 11.55 & 473.96 & 497.65 & 522.54 & 548.66 \\
      Fork, NoPK  & 0.16 & 0.15 & 0.16 & 0.16 & 120.86 & 121.10 & 122.06 & 121.82 \\
      Diff, NoPK  & 0.59 & 7.95 & 8.69 & 61.07 & 445.42 & 448.96 & 456.04 & 454.27 \\
      Merge, NoPK  & 0.58 & 8.05 & 10.04 & 66.74 & 448.30 & 451.87 & 459.00 & 457.21 \\
      \bottomrule
    \end{tabular}
  }
\end{table}
...
\begin{figure}[t]
  \centering
  \input{fig_collab_merge_conflict.tex}
  \caption{Merge time across change sets for collaborative workload with conflicts.}
  \label{fig:collab-merge-conflict-plot}
\end{figure}

\subsection{Discussion}
We have conducted experiments that simulate some typical data 
workflows of a data engineer or a team of data engineers. 
